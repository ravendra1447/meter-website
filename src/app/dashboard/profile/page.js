...
export default function ProfilePage() {
  ...
  const [user, setUser] = useState(() => {
    let initialUser = { name: 'Admin User', mobile: 'N/A', email: 'N/A', role: 'master' };
    if (typeof window !== 'undefined') {
      ...
      if (userDataStr) {
        try {
          ...
          initialUser = {
            name: userData.name || userData.first_name || 'Admin User',
            mobile: userData.mobile || userData.phone || 'N/A',
            email: userData.email || 'N/A',
            role: localStorage.getItem('user_role') || 'master'
          };
        } catch (e) {}
      } else {
        initialUser.role = localStorage.getItem('user_role') || 'master';
      }
    }
    return initialUser;
  });
        ...
        <div className="relative z-10 p-8 sm:p-10 flex flex-col sm:flex-row items-center sm:items-start gap-6 sm:gap-8">
          <div className="w-28 h-28 rounded-full bg-white/20 p-2 backdrop-blur-md shadow-xl shrink-0">
            <div className="w-full h-full rounded-full bg-white flex items-center justify-center">
              <span className="text-4xl font-bold text-transparent bg-clip-text bg-gradient-to-br from-blue-600 to-purple-600">
                {user.name.charAt(0)}
              </span>
            </div>
          </div>
          ...
          <div className="flex sm:flex-col gap-3 w-full sm:w-auto mt-4 sm:mt-0">
            <button onClick={() => router.push('/dashboard/profile/edit')} className="flex-1 sm:flex-none flex items-center justify-center gap-2 px-5 py-2.5 bg-white text-blue-600 rounded-xl font-semibold hover:bg-blue-50 transition-colors shadow-lg active:scale-95">
              <Edit size={18} /> Edit
            </button>
          </div>
        </div>
