...
export default function OwnersPage() {
  ...
  useEffect(() => {
    const fetchOwners = async () => {
      try {
        const response = await api.get('/master/owners');
        // The API returns paginated data inside response.data.data
          ...
          <table className="w-full text-sm text-left">
            ...
            <tbody>
              {loading ? (
                ...
                owners.map((owner) => (
                  <tr key={owner.id} className="border-b border-[var(--border)] hover:bg-[var(--accent)]/50 transition-colors">
                    <td className="px-6 py-4 font-medium">{owner.name}</td>
                    <td className="px-6 py-4">{owner.mobile}</td>
                    <td className="px-6 py-4">{owner.owned_properties_count || 0} Properties</td>
