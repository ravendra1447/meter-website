...
export default function EditProfilePage() {
  ...
  const handleSubmit = async (e) => {
    ...
    setTimeout(() => {
      ...
      setTimeout(() => router.push('/tenant/profile'), 1500);
    }, 1000);
  };
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <div className="flex items-center gap-4">
        <Link href="/tenant/profile" className="p-2 bg-[var(--card)] border border-[var(--border)] rounded-xl hover:bg-[var(--accent)] transition-colors text-[var(--muted-foreground)]">
          <ArrowLeft size={20} />
        </Link>
        <div>
      ...
      <div className="bg-[var(--card)] rounded-3xl border border-[var(--border)] shadow-sm overflow-hidden">
        <div className="bg-[var(--accent)]/50 px-8 py-6 border-b border-[var(--border)] flex items-center gap-4">
          <div className="w-16 h-16 rounded-full bg-gradient-to-tr from-orange-500 to-red-500 flex items-center justify-center text-white text-2xl font-bold shadow-md">
        ...
        <form onSubmit={handleSubmit} className="p-8 space-y-6">
          ...
          <div className="pt-4 flex justify-end gap-4">
            <Link href="/tenant/profile" className="px-6 py-3 rounded-xl border border-[var(--border)] hover:bg-[var(--accent)] font-semibold transition-colors">
              Cancel
            </Link>
            <button 
              type="submit" 
              disabled={loading}
              className="flex items-center gap-2 px-8 py-3 bg-gradient-to-r from-orange-600 to-red-600 text-white rounded-xl font-bold hover:shadow-lg hover:shadow-orange-500/25 transition-all duration-300 disabled:opacity-50 active:scale-95"
            >
              {loading ? (
