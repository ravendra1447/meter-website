...
export default function ProfilePage() {
  ...
  const [user, setUser] = useState(() => {
    let initialUser = { name: 'Owner User', mobile: 'N/A', email: 'N/A', role: 'owner' };
    if (typeof window !== 'undefined') {
      ...
      if (userDataStr) {
        try {
          ...
          initialUser = {
            name: userData.name || userData.first_name || 'Owner User',
        ...
        initialUser.role = localStorage.getItem('user_role') || 'owner';
      }
    }
    return initialUser;
  });
    ...
    return (
      ...
      >
        <div className="flex items-center gap-4">
          <div className={`p-3 rounded-xl ${isDanger ? 'bg-red-500/10 text-red-500' : 'bg-blue-500/10 text-blue-500 group-hover:scale-110 transition-transform'}`}>
            <Icon size={20} />
          </div>
          <div>
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500 pb-20">
      ...
      <div className="relative rounded-3xl overflow-hidden shadow-lg border border-[var(--border)]">
        {/* Background Gradient */}
        <div className="absolute inset-0 bg-gradient-to-br from-green-600 via-teal-600 to-blue-700 opacity-90"></div>
        {/* Decorative Circles */}
        ...
        <div className="relative z-10 p-8 sm:p-10 flex flex-col sm:flex-row items-center sm:items-start gap-6 sm:gap-8">
          <div className="w-28 h-28 rounded-full bg-white/20 p-2 backdrop-blur-md shadow-xl shrink-0">
            <div className="w-full h-full rounded-full bg-white flex items-center justify-center">
              <span className="text-4xl font-bold text-transparent bg-clip-text bg-gradient-to-br from-green-600 to-teal-600">
                {user.name.charAt(0)}
              </span>
            </div>
          </div>
          ...
          <div className="flex sm:flex-col gap-3 w-full sm:w-auto mt-4 sm:mt-0">
            <button onClick={() => router.push('/owner/profile/edit')} className="flex-1 sm:flex-none flex items-center justify-center gap-2 px-5 py-2.5 bg-white text-green-600 rounded-xl font-semibold hover:bg-green-50 transition-colors shadow-lg active:scale-95">
              <Edit size={18} /> Edit
            </button>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pt-4">
        ...
        {/* Column 1: Management & Controls */}
        <div className="space-y-4">
          <h2 className="text-lg font-bold text-[var(--muted-foreground)] px-1 uppercase tracking-wider text-sm">Management</h2>
          <div className="space-y-3">
            <ActionCard 
              icon={Building} 
              title="Manage Properties" 
              subtitle="Add or edit your buildings & flats"
              onClick={() => router.push('/owner/properties')}
            />
            <ActionCard 
              icon={Bluetooth} 
              title="Manage Meters" 
              subtitle="View and install smart meters"
              onClick={() => router.push('/owner/meters')}
            />
          </div>
        </div>
        ...
        <div className="space-y-4">
          <h2 className="text-lg font-bold text-[var(--muted-foreground)] px-1 uppercase tracking-wider text-sm">Settings & Activity</h2>
          <div className="space-y-3">
            <ActionCard 
              icon={Lock} 
              title="Change Password" 
              subtitle="Update your security credentials"
              onClick={() => router.push('/owner/profile/change-password')}
            />
            <ActionCard 
              icon={History} 
              title="Activity Logs" 
              subtitle="View recent actions and events"
              onClick={() => {}}
            />
            <ActionCard 
              icon={Settings} 
