...
export default function SettingsPage() {
  ...
  const ToggleSwitch = ({ checked, onChange }) => (
    <button 
      type="button" 
      onClick={onChange}
      className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors focus:outline-none focus:ring-2 focus:ring-green-500 focus:ring-offset-2 ${checked ? 'bg-green-600' : 'bg-gray-300 dark:bg-gray-700'}`}
    >
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500 pb-20">
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        {/* Preferences Section */}
        <div className="bg-[var(--card)] rounded-2xl border border-[var(--border)] shadow-sm overflow-hidden group hover:border-teal-500/30 transition-colors">
          <div className="bg-[var(--accent)]/50 px-6 py-4 border-b border-[var(--border)] flex items-center gap-3">
            ...
            <div className="space-y-1.5">
              ...
              <div className="relative">
                ...
                <select 
                  ...
                  className="w-full bg-[var(--background)] border border-[var(--border)] rounded-xl pl-11 pr-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-teal-500/50 transition-all appearance-none"
                >
